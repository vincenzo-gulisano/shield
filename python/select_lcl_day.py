...
class DailyAggregate:
    ...
    finite_count: int = 0
    row_count: int = 0
    half_hour_values: list[float] | None = None
    ...
    def add_value(self, value: float) -> None:
        ...
        self.half_hour_values.append(value)
...
def collect_daily_rows(
    ...
    for (row_day, lcl_id), aggregate in aggregates.items():
        ...
        if not aggregate.half_hour_values:
            ...
            max_value = format_number(max(aggregate.half_hour_values))
            median_value = format_number(median(aggregate.half_hour_values))
        rows.append(
            (
                timestamp_ms,
                lcl_id,
...
def write_output(
    ...
    header = ["LCLid", "stdorToU", timestamp_column, value_column, OUTPUT_MAX_COLUMN, OUTPUT_MEDIAN_COLUMN]
...
def main() -> int:
    ...
    try:
        ...
        if args.list_days:
            ...
            return 0
        if args.max_keys is None or args.max_keys < 1:
            raise ValueError("Use --max-keys with a positive integer")

        start_day, end_day = resolve_period(args)
        rows, selected_keys = collect_daily_rows(files, start_day, end_day, args.max_keys, timezone)
        if not rows:
            raise ValueError(
        ...
        write_output(rows, output_path, args.timestamp_column, args.value_column)
